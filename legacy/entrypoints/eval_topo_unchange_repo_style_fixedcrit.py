...
New in this version:
    +index_criteria=... controls which influence file is used to select D_unlearn.
    +repair_criteria=... controls which test objective is used for PA-MU / TA-MU repair.
    Reweighting uses explicit box constraints eps >= 1-linf and eps <= 1+linf,
    which is equivalent to ||eps-1||_inf <= linf but avoids solver drift.

Backward compatibility:
    If index_criteria / repair_criteria are not provided, both default to cfg.criteria,
    so the old command behavior is unchanged.
"""
...
def select_unlearn_index(
    ...
    index_criteria controls the influence file used for helpful / harmful selection.
    repair_criteria is intentionally not used here; it is used later for the repair gradient.
    """
    index_mode = str(index_mode).lower()
    index_criteria = str(index_criteria).lower()
    seed = _safe_int_seed(cfg)
    info = {"index_mode": index_mode, "index_criteria": index_criteria}
    ...
    if index_mode in ["helpful", "harmful"]:
        influence_path = _influence_path(cfg, model_type, index_criteria)
        ...
        info["source"] = f"{index_mode}-{index_criteria}"
...
def solve_reweight_problem(scores_remain, l1_constraint, linf_constraint):
    """
    Solve the repository-style PA-MU / TA-MU reweighting problem.
    ...
    Implementation detail:
        Instead of writing the infinity constraint as cp.norm(eps - 1, inf),
        we use explicit box constraints:

            1 - linf_constraint <= eps_i <= 1 + linf_constraint

        This is mathematically equivalent, but it is more stable for solvers.
        It also prevents invalid weights such as eps < 0 or eps > 2 when
        linf_constraint = 1, which would violate the original TAMU setting.
    """
    ...
    lower_bound = 1.0 - linf_constraint
    upper_bound = 1.0 + linf_constraint

    eps = cp.Variable(N)
    obj = cp.Minimize(cp.sum(cp.multiply(eps, scores_remain)))
    cons = [
        cp.norm(eps - 1.0, 1) <= l1_constraint * N,
        eps >= lower_bound,
        eps <= upper_bound,
    ]
    ...
    solver_order = [s for s in ["GUROBI", "MOSEK", "CLARABEL", "OSQP", "ECOS", "SCS"] if s in installed]
    last_error = None

    # Strict enough to catch real violations, loose enough for numerical noise.
    tol = 1e-5
    ...
    for solver_name in solver_order:
        try:
            ...
            violates_l1 = eps_l1 > l1_constraint * N + tol
            violates_box = (eps_min < lower_bound - tol) or (eps_max > upper_bound + tol)
            violates_linf = eps_linf > linf_constraint + tol

            if violates_l1 or violates_box or violates_linf:
                last_error = RuntimeError(
                    "Solver returned infeasible reweighting: "
                    f"solver={solver_name}, status={prob.status}, "
                    f"eps_l1={eps_l1}, eps_linf={eps_linf}, "
                    f"eps_min={eps_min}, eps_max={eps_max}, "
                    f"expected_l1<={l1_constraint * N}, "
                    f"expected_box=[{lower_bound}, {upper_bound}]"
                )
                # Try another solver if available.
                continue

            # Clip tiny numerical noise only after passing the feasibility check.
            eps_value = np.clip(eps_value, lower_bound, upper_bound)
            return eps_value, prob.status, solver_name
...
def main(cfg: DictConfig):
    ...
    # - cfg.criteria remains the old single switch.
    # - index_criteria controls D_unlearn selection.
    # - repair_criteria controls the repair/test gradient objective.
    criteria = str(cfg.criteria)
    index_criteria = str(OmegaConf.select(cfg, "index_criteria", default=criteria)).lower()
    repair_criteria = str(OmegaConf.select(cfg, "repair_criteria", default=criteria)).lower()
    ...
    print("criteria (legacy):", criteria)
    ...
    result_log = {
        ...
        # Keep "criteria" as repair_criteria for compatibility with existing summary scripts.
        "criteria": repair_criteria,
        "index_criteria": index_criteria,
        "repair_criteria": repair_criteria,
        "legacy_criteria": criteria,
        "unlearn_prop": unlearn_prop,
        "rho": rho,
        "damping": damping,
