...
class AutogradInfluenceModule(BaseInfluenceModule):
    ...
    def __init__(
        ...
        # print('size of train_loader', len(train_loader.dataset))
        self.damp = damp
        self.gnh = gnh
        ...
        #     self._model_reinsert_params(self._reshape_like_params(flat_params))
        #     print('train loss')
        #     print(self.objective.train_loss(self.model, flat_params, batch))

        print("calculating the hessian...")
        for batch, batch_size in tqdm(self._loader_wrapper(train=True), total = len(train_loader)):
            # a new dataloader on the train loader
            start_time = time.time()
            # consider the gnh option, simply copy the _hvp_at_batch function here
            def f(theta_):
            ...
            if gnh:
                # # output
                # y_batch = out_f(flat_params)
                # # print("y_batch finished")
                # # hessian
                # hess_cost_y = torch.autograd.functional.hessian(loss_f, y_batch).detach()
                # hess_cost_y_batch = hess_cost_y.sum(2) * batch_size
                # # print("hessian finished")
                # # jacobian
                # jacobian_y_theta_batch = torch.autograd.functional.jacobian(out_f, flat_params).detach()
                # hess_batch = torch.mean(torch.matmul(torch.matmul(jacobian_y_theta_batch.transpose(1, 2), hess_cost_y_batch), jacobian_y_theta_batch), axis = 0) + torch.autograd.functional.hessian(reg_f, flat_params).detach()
                print("dont allow gnh")
            else:
                hess_batch = torch.autograd.functional.hessian(f, flat_params).detach() # both train loss and regularization loss
        ...
        with torch.no_grad():
            ...
            hess = hess / len(self.train_loader.dataset) # average over the dataset given by train
    ...
    def inverse_hvp(self, vec):
        self.inverse_hess = torch.inverse(self.hessian)
...
class CGInfluenceModule(BaseInfluenceModule):
    ...
    def __init__(
        ...
        self.last_cg_residual_norm = None
        self.last_cg_relative_residual = None
    ...
    def inverse_hvp(self, vec):
        ...
        if self.watch_progress:
            ihvp, info = L.cg(A=linop, b=vec.cpu().numpy(), callback=report, **self.cg_kwargs)
        else:
            ihvp, info = L.cg(A=linop, b=vec.cpu().numpy(), callback=report, **self.cg_kwargs)

        residual = hvp_fn(ihvp) - vec.cpu().numpy()
        self.last_cg_info = int(info)
        self.last_cg_iterations = int(iteration_counter["count"])
        self.last_cg_residual_norm = float(np.linalg.norm(residual))
        self.last_cg_relative_residual = float(
            np.linalg.norm(residual) / (np.linalg.norm(vec.cpu().numpy()) + 1e-12)
        )

        with torch.no_grad():
            self._model_reinsert_params(self._reshape_like_params(flat_params), register=True)

        print("cg info (0 for success): {}".format(info))
...
class LiSSAInfluenceModule(BaseInfluenceModule):
    ...
    def __init__(
            ...
            test_loader: data.DataLoader,
            device: torch.device,
            damp: float,  # the damping strength for positive definite matrix
            repeat: int,  # sample R times
            depth: int,   # sample batch numbers
            scale: float, # to ensure the convergence, should be sufficiently large
            gnh: bool = False,
    ...
    def inverse_hvp(self, vec):
        ...
        for r in range(self.repeat):
            ...
            for t, (batch, _) in enumerate(self._loader_wrapper(sample_n_batches=self.depth, train=True)):
                # recursive update using neumann series
                # update the vector h_est
                ...
                print("h_est norm", torch.norm(h_est, p=2))
                h_est_ori = h_est.clone()
